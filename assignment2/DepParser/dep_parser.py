...
class Parser: 
    ...
    def compute_correct_move(self, i,stack,pred_tree,correct_tree) :
        ...
        # YOUR CODE HERE
        moves = self.valid_moves(i, stack, pred_tree)
        if Parser.LA in moves and correct_tree[stack[-2]] == stack[-1]:
            return Parser.LA
        
        elif Parser.RA in moves and correct_tree[stack[-1]] == stack[-2]:
            for j in range(len(pred_tree)):
                if correct_tree[j] == stack[-1] and pred_tree[j] != stack[-1]:
                    if Parser.SH in moves:
                        return Parser.SH
                    else:
                        return None
            return Parser.RA
        
        elif Parser.SH in moves:
            return Parser.SH
